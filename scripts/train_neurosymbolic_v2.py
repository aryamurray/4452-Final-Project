"""Train MiniResNetJointV2 with 3-phase neurosymbolic training.

V2 reranker improvements over v1:
  - Soft radical features (sigmoid probs, not hard binary threshold)
  - Zero-masked stroke type cosine (handles missing stroke signatures)
  - Neural confidence as 6th MLP feature
  - top_k: 5 → 20, hidden_dim: 32 → 64, reranker_weight init: 0.01 → 0.1
...
Usage:
    python scripts/train_neurosymbolic_v2.py \\
        --train-dir data/HWDB1.1/train \\
        --epochs-phase1 50 --epochs-phase2 30 --epochs-phase3 10

    # Warm-start from v1 phase-1 checkpoint (saves ~50 epochs):
    python scripts/train_neurosymbolic_v2.py \\
        --pretrained-backbone outputs/neurosymbolic/best_model_phase1.pt \\
        --resume-phase 2 --wandb --wandb-project hccr-neurosymbolic-v2
"""
...
from hccr.models.mini_resnet_joint_v2 import MiniResNetJointV2
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("outputs/neurosymbolic_v2"))
    ...
    parser.add_argument("--wandb-project", type=str, default="hccr-neurosymbolic-v2")
    ...
    logger.info("=" * 60)
    logger.info("Neurosymbolic Training: MiniResNetJointV2 (3-phase, improved reranker)")
    logger.info("=" * 60)
    ...
    if resume_phase and resume_phase >= 2:
        ...
        model.reranker.reranker_weight.data.fill_(0.1)
